      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg pl-10 pr-4 py-2.5 text-sm text-gray-900 dark:text-white placeholder-gray-500 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none"
          />
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-x-auto">
        <table className="w-full text-sm min-w-[750px]">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {filtered.map(g => (
              <tr key={g.id} className="hover:bg-gray-100 dark:hover:bg-gray-700/30 transition-colors">
                <td className="px-5 py-3">
                  <div className="flex items-center gap-3">
                    <div className="h-9 w-9 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center text-gray-900 dark:text-white text-sm font-bold flex-shrink-0">
                      {g.name.split(' ').map(n => n[0]).join('')}
                    </div>
                    <div>
                      <p className="text-gray-900 dark:text-white font-medium flex items-center gap-2">
                        {g.name}
                        {g.vip && <span className="bg-amber-500/20 text-amber-600 dark:text-amber-400 text-[10px] px-1.5 py-0.5 rounded font-bold">VIP</span>}
                      </p>
                ...
                <td className="px-5 py-3">
                  <p className="text-gray-600 dark:text-gray-300 flex items-center gap-1.5 text-xs"><Mail className="h-3 w-3 text-gray-500" /> {g.email}</p>
                  <p className="text-gray-500 dark:text-gray-400 flex items-center gap-1.5 text-xs mt-1"><Phone className="h-3 w-3 text-gray-500" /> {g.phone}</p>
                </td>
                <td className="px-5 py-3">
                  <span className="text-gray-600 dark:text-gray-300 flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5 text-gray-500" /> {g.country}</span>
                </td>
                <td className="px-5 py-3 text-gray-600 dark:text-gray-300 text-center">{g.visits}</td>
                <td className="px-5 py-3 text-gray-500 dark:text-gray-400">{g.lastVisit}</td>
                <td className="px-5 py-3 text-gray-900 dark:text-white font-medium">{g.totalSpent}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 w-full max-w-md">
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-gray-900 dark:text-white font-semibold">Add New Guest</h3>
            ...
            <div className="p-4 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-gray-100 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-amber-500 outline-none"
                  />
                </div>
                <div>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-amber-500 focus:ring-amber-500 bg-gray-100 dark:bg-gray-700"
                />
                <span className="text-sm text-gray-600 dark:text-gray-300">VIP Guest</span>
              </label>
            </div>
