    ...
    <div className="space-y-4">
      <div className="bg-gray-800 rounded-xl p-4 border border-gray-700 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-lg bg-red-500/20"><Heart className="h-5 w-5 text-red-400" /></div>
          <div><p className="text-sm text-gray-400">Total Photo Likes</p><p className="text-xl font-bold text-white">{galleryStats.totalLikes}</p></div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="flex gap-2 flex-wrap">
          {categories.map(c => (<button key={c} onClick={() => setCategory(c)} className={`px-3 py-2 rounded-lg text-xs font-medium transition-colors ${category === c ? 'bg-amber-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white border border-gray-700'}`}>{c}</button>))}
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {selected.size > 0 && (<button onClick={() => openModal('delete')} className="flex items-center gap-1.5 bg-red-500/20 text-red-400 hover:bg-red-500/30 text-xs font-medium px-3 py-2 rounded-lg transition-colors"><Trash2 className="h-3.5 w-3.5" /> Delete ({selected.size})</button>)}
          <div className="flex bg-gray-800 border border-gray-700 rounded-lg overflow-hidden">
            <button onClick={() => setViewMode('grid')} className={`p-2 ${viewMode === 'grid' ? 'bg-gray-700 text-white' : 'text-gray-500 hover:text-white'}`}><Grid className="h-4 w-4" /></button>
            <button onClick={() => setViewMode('list')} className={`p-2 ${viewMode === 'list' ? 'bg-gray-700 text-white' : 'text-gray-500 hover:text-white'}`}><List className="h-4 w-4" /></button>
          </div>
      ...
      {viewMode === 'grid' ? (
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(img => (
            <div key={img.id} className={`bg-gray-800 rounded-xl border overflow-hidden cursor-pointer group transition-all ${selected.has(img.id) ? 'border-amber-500 ring-2 ring-amber-500/30' : 'border-gray-700 hover:border-gray-600'}`} onClick={() => toggleSelect(img.id)}>
              <div className="aspect-[4/3] relative overflow-hidden">
                ...
                <div className={`absolute top-2 left-2 h-5 w-5 rounded border-2 flex items-center justify-center transition-colors ${selected.has(img.id) ? 'bg-amber-500 border-amber-500' : 'border-white/50 bg-black/20'}`}>{selected.has(img.id) && <span className="text-white text-xs">✓</span>}</div>
              </div>
              <div className="p-3"><div className="flex items-center justify-between"><div className="flex-1 min-w-0"><p className="text-white text-sm font-medium truncate">{img.name}</p><p className="text-gray-500 text-xs">{img.category} · {img.size}</p></div>{(likeCounts[img.numericId] || 0) > 0 && (<div className="flex items-center gap-1 text-red-400 ml-2"><Heart className="h-3.5 w-3.5 fill-red-400" /><span className="text-xs font-medium">{likeCounts[img.numericId]}</span></div>)}</div></div>
            </div>
          ))}
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-gray-700">
              {filtered.map(img => (
                <tr key={img.id} className="hover:bg-gray-700/30 transition-colors">
                  <td className="px-5 py-3"><button onClick={() => toggleSelect(img.id)} className={`h-5 w-5 rounded border-2 flex items-center justify-center ${selected.has(img.id) ? 'bg-amber-500 border-amber-500' : 'border-gray-600'}`}>{selected.has(img.id) && <span className="text-white text-xs">✓</span>}</button></td>
                  <td className="px-5 py-3"><div className="flex items-center gap-3"><img src={img.url} alt={img.name} className="h-10 w-14 object-cover rounded" /><span className="text-white">{img.name}</span></div></td>
                  <td className="px-5 py-3 text-gray-400">{img.category}</td>
                  <td className="px-5 py-3">{(likeCounts[img.numericId] || 0) > 0 ? (<div className="flex items-center gap-1 text-red-400"><Heart className="h-3.5 w-3.5 fill-red-400" /><span className="text-xs font-medium">{likeCounts[img.numericId]}</span></div>) : (<span className="text-gray-600 text-xs">0</span>)}</td>
                  <td className="px-5 py-3 text-gray-400">{img.size}</td>
                  <td className="px-5 py-3 text-gray-400">{img.uploadDate}</td>
                  <td className="px-5 py-3 text-right"><button onClick={() => openModal('view', img)} className="text-gray-400 hover:text-white p-1.5 rounded-lg hover:bg-gray-700 transition-colors"><Eye className="h-4 w-4" /></button><button onClick={() => { setSelected(new Set([img.id])); openModal('delete'); }} className="text-gray-400 hover:text-red-400 p-1.5 rounded-lg hover:bg-gray-700 transition-colors ml-1"><Trash2 className="h-4 w-4" /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {modalType === 'upload' && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-800 rounded-xl border border-gray-700 w-full max-w-md">
            <div className="flex items-center justify-between p-4 border-b border-gray-700"><h3 className="text-white font-semibold">Upload Images</h3><button onClick={closeModal} className="text-gray-400 hover:text-white"><X className="h-5 w-5" /></button></div>
            <div className="p-4 space-y-4">
              <div className="border-2 border-dashed border-gray-600 rounded-xl p-8 text-center hover:border-amber-500 transition-colors cursor-pointer">
              ...
              <div><label className="block text-sm text-gray-400 mb-1">Image Name</label><input type="text" value={uploadForm.name} onChange={e => setUploadForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g., Pool sunset view" className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-amber-500 outline-none" /></div>
              <div><label className="block text-sm text-gray-400 mb-1">Category</label><select value={uploadForm.category} onChange={e => setUploadForm(p => ({ ...p, category: e.target.value }))} className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-amber-500 outline-none">{categories.filter(c => c !== 'All').map(c => <option key={c}>{c}</option>)}</select></div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-800 rounded-xl border border-gray-700 w-full max-w-sm">
            <div className="p-6 text-center">
              ...
              <h3 className="text-white font-semibold text-lg mb-2">Delete {selected.size} Image{selected.size > 1 ? 's' : ''}?</h3>
              <p className="text-gray-400 text-sm mb-6">This action cannot be undone.</p>
              <div className="flex gap-3 justify-center"><button onClick={closeModal} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white text-sm font-medium rounded-lg transition-colors">Cancel</button><button onClick={() => { setSelected(new Set()); closeModal(); }} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition-colors">Delete</button></div>
            </div>
          </div>
        </div>
      )}
