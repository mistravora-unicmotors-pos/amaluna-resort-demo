    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search inquiries..." className="w-full bg-gray-800 border border-gray-700 rounded-lg pl-10 pr-4 py-2.5 text-sm text-white placeholder-gray-500 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none" />
        </div>
        <div className="flex gap-2 flex-wrap">
          {['All', 'New', 'Read', 'Replied', 'Closed'].map(f => (
            <button key={f} onClick={() => setFilter(f)} className={`px-3 py-2 rounded-lg text-xs font-medium transition-colors ${filter === f ? 'bg-amber-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white border border-gray-700'}`}>{f}</button>
          ))}
        </div>
      </div>
            ...
            {filtered.map(inq => {
              ...
              return (
                <tr key={inq.id} className="hover:bg-gray-700/30 transition-colors">
                  ...
                  <td className="px-5 py-3"><p className="text-white">{inq.name}</p><p className="text-gray-500 text-xs">{inq.email}</p></td>
                  ...
                  <td className="px-5 py-3 text-right">
                    <button onClick={() => openModal('view', inq)} className="text-gray-400 hover:text-white p-1.5 rounded-lg hover:bg-gray-700 transition-colors"><Eye className="h-4 w-4" /></button>
                    <button onClick={() => openModal('reply', inq)} className="text-gray-400 hover:text-amber-400 p-1.5 rounded-lg hover:bg-gray-700 transition-colors ml-1"><MessageSquare className="h-4 w-4" /></button>
                    <button onClick={() => openModal('delete', inq)} className="text-gray-400 hover:text-red-400 p-1.5 rounded-lg hover:bg-gray-700 transition-colors ml-1"><Trash2 className="h-4 w-4" /></button>
                  </td>
                </tr>
              );
            })}
      ...
      {modalType === 'view' && selectedInquiry && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-800 rounded-xl border border-gray-700 w-full max-w-lg">
            <div className="flex items-center justify-between p-4 border-b border-gray-700"><h3 className="text-white font-semibold">Inquiry Details</h3><button onClick={closeModal} className="text-gray-400 hover:text-white"><X className="h-5 w-5" /></button></div>
            <div className="p-4 space-y-4">
              <div className="flex justify-between items-start"><div><p className="text-white font-medium">{selectedInquiry.name}</p><p className="text-gray-400 text-sm">{selectedInquiry.email}</p></div><span className={`px-2.5 py-1 rounded-full text-xs font-medium ${statusConfig[selectedInquiry.status].color}`}>{selectedInquiry.status}</span></div>
              <div><p className="text-gray-500 text-xs mb-1">Subject</p><p className="text-white">{selectedInquiry.subject}</p></div>
              <div><p className="text-gray-500 text-xs mb-1">Message</p><div className="bg-gray-700/50 rounded-lg p-3"><p className="text-gray-300 text-sm">{selectedInquiry.message}</p></div></div>
              <p className="text-gray-500 text-xs">Received: {selectedInquiry.date}</p>
            </div>
            <div className="flex justify-end gap-2 p-4 border-t border-gray-700"><button onClick={closeModal} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white text-sm font-medium rounded-lg transition-colors">Close</button><button onClick={() => { closeModal(); openModal('reply', selectedInquiry); }} className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white text-sm font-medium rounded-lg transition-colors">Reply</button></div>
          </div>
        </div>
      )}
      ...
      {modalType === 'reply' && selectedInquiry && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-800 rounded-xl border border-gray-700 w-full max-w-lg">
            <div className="flex items-center justify-between p-4 border-b border-gray-700"><h3 className="text-white font-semibold">Reply to Inquiry</h3><button onClick={closeModal} className="text-gray-400 hover:text-white"><X className="h-5 w-5" /></button></div>
            <div className="p-4 space-y-4">
              <div className="bg-gray-700/30 rounded-lg p-3"><div className="flex justify-between items-center mb-2"><span className="text-white font-medium text-sm">{selectedInquiry.name}</span><span className="text-gray-500 text-xs">{selectedInquiry.date}</span></div><p className="text-gray-400 text-sm font-medium mb-1">{selectedInquiry.subject}</p><p className="text-gray-300 text-sm">{selectedInquiry.message}</p></div>
              <div><label className="block text-sm text-gray-400 mb-1">To: {selectedInquiry.email}</label><textarea rows={5} value={replyText} onChange={e => setReplyText(e.target.value)} placeholder="Type your reply..." className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-amber-500 outline-none resize-none" /></div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-800 rounded-xl border border-gray-700 w-full max-w-sm">
            <div className="p-6 text-center">
              ...
              <p className="text-gray-400 text-sm mb-6">Are you sure you want to delete inquiry from <span className="text-white font-medium">{selectedInquiry.name}</span>?</p>
